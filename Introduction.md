...
Unification is the process of finding a substitution, called «unifier», for variables that make two terms equal. For example, a substitution `[X -> a]` is a unifier for two terms `f (a)` and `f (X)`, whereas the terms `g (Y b)` and `g (c  d)` can’t be unified. 
